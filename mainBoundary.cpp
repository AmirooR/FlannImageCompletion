...
int smoothFn(int p1, int p2, int l1, int l2)
{
        ...
        //int retMe1 = smoothFn2(p1,p2,l2,l1);
...
int dataFn(int p, int l, void *data)
{
	if( l == topNum - 1)
	{
		if(isValid(gcNodes[p].x, gcNodes[p].y))
		{
			return 0;
		}
		else
...
vector<Point2i> maskToNodes(cv::Mat mask)
{
        ...
        for(int y = 1; y < mask.rows - 1; y++)
        {
                ...
                {
                        if( mask.at<uchar>(y, x) == 0 || mask.at<uchar>(y,x-1) == 0 || mask.at<uchar>(y-1,x) == 0 || mask.at<uchar>(y,x+1) == 0 || mask.at<uchar>(y+1,x) == 0)
                        {
...
int main()
{
        ...
        char* img_path = "/Users/amirrahimi/Downloads/Applications/cvpr10Data/images/img-op1-p-282t000-resized.jpg";
        char* mask_path = "/Users/amirrahimi/Desktop/TMP/OccMasks/img-op1-p-282t000_mask.png";
        ...
        try
        {
                ...
                gc->swap(10);
                cout<<"Energy After: "<<gc->compute_energy()<<endl;
                for(int _i = 0; _i < gcNodes.size(); _i++)
                {
                    int res = gc->whatLabel(_i);
		    Point2i newP =gcNodes[_i]+gcLabels[res]; 
		    if( isInside(newP.x, newP.y) )
		    {
			    //cout<<"\t\tPoint: "<<gcNodes(_i)<<
			    for(int _nch = 0; _nch < 3; _nch++)
				    img.at<Vec3b>(gcNodes[_i])[_nch] = img.at<Vec3b>(gcNodes[_i]+gcLabels[res])[_nch];
		    }
		    else
		    {
			    cout<<"\t\tInvalid Point: ("<<newP.x<<" , "<<newP.y<<") = ("<<gcNodes[_i].x<<","<<gcNodes[_i].y<<") + (";
			    cout<<gcLabels[res].x<<", "<<gcLabels[res].y<<")"<<endl;
			    cout<<"\t\t\t label is : "<<res<<endl;
		    }
